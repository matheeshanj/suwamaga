...
// ─── SearchScreen — searches articles across all categories ──────────────────
function SearchScreen({ query, onBack, onSelect }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // Search by title_si, title_en, summary across all articles
    Promise.all([
  ...
  const [selected, setSelected] = useState(null);
  if (selected) return <ArticleDetailScreen article={selected} onBack={() => setSelected(null)} />;
  ...
  return (
    <div style={{ fontFamily:"Noto Sans Sinhala,system-ui,sans-serif", background:T.bg, minHeight:"100vh", maxWidth:480, margin:"0 auto", position:"relative", paddingBottom:72 }}>
      <div style={{ padding:"16px 16px 0" }}>
        ...
        {screen.type === "search" && (
          <SearchScreen query={screen.query} onBack={goBack} onSelect={a => navigate("article_direct", { articleId: a.id })} />
        )}
