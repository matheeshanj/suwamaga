    ...
    return (
      <div>
        ...
        <SectionCard title={editing === "new" ? "✏️ New Article" : "✏️ Edit Article"}>
          ...
          <Textarea label="SUMMARY (1–2 lines for list view)" value={form.summary} onChange={f("summary")} rows={2} placeholder="Short description shown in category list..." hint="Shown under title in article list" />
          <Textarea label={c === "medicines" ? "මේ ඖෂධය ශරීරය තුළ ක්‍රියා කරන්නේ කෙසේද? (What it is & How it Works)" : "OVERVIEW"} value={form.overview} onChange={f("overview")} rows={4} placeholder="Main explanation paragraph..." hint="First thing the reader sees" />
          {show.symptoms && <Textarea label={labels.symptoms[c] || "ලක්ෂණ — one per line"} value={form.symptoms} onChange={f("symptoms")} placeholder={"item 1\nitem 2\nitem 3"} />}
          {show.causes && <Textarea label={labels.causes[c] || "හේතු — one per line"} value={form.causes} onChange={f("causes")} placeholder={"item 1\nitem 2\nitem 3"} />}
          {show.warning_signs && <Textarea label={labels.warning_signs[c] || "⚠️ අනතුරු ලකුණු — one per line"} value={form.warning_signs} onChange={f("warning_signs")} placeholder={"warning 1\nwarning 2"} />}
          ...
          {show.selfcare && <Textarea label={labels.selfcare[c] || "ස්ව-රැකවරණ — one per line"} value={form.selfcare} onChange={f("selfcare")} placeholder={"item 1\nitem 2\nitem 3"} />}
          {show.prevention && <Textarea label={labels.prevention[c] || "වැළැක්වීම — one per line"} value={form.prevention} onChange={f("prevention")} placeholder={"item 1\nitem 2\nitem 3"} />}
          {show.see_doctor && <Textarea label={labels.see_doctor[c] || "වෛද්‍යවරයෙකු හමුවිය යුත්තේ කවදාද?"} value={form.see_doctor} onChange={f("see_doctor")} rows={2} placeholder="Describe when to seek urgent care..." hint="Free text — not a list" />}
          ...
          <CheckboxField label="FEATURED?" checked={form.featured} onChange={v => setForm(p => ({ ...p, featured: v }))} caption="Show on home screen" />
  ...
  if (editing !== null) return (
    <div>
      ...
      <SectionCard title={editing === "new" ? "➕ New Specialist" : "✏️ Edit Specialist"}>
        ...
        <Input label="DIVISION / AREA" value={form.division} onChange={f("division")} placeholder="Colombo 08" />
